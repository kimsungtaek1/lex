class AssetManager {
  constructor() {
    ...
    this.currentAssets = {
      ...
      seizure_deposit: [], // (가)압류 적립금 추가
      deposit_money: []    // 공탁금 추가
    };
    ...
    this.assetCounters = {
      ...
      seizure_deposit: 0, // (가)압류 적립금 카운터 추가
      deposit_money: 0    // 공탁금 카운터 추가
    };
  ...
  initializeEventHandlers() {
    ...
    $("#add_seizure_deposit_asset").on("click", () => this.addSeizureDepositBlock()); // (가)압류 적립금 추가 버튼 이벤트
    $("#add_deposit_money_asset").on("click", () => this.addDepositMoneyBlock());     // 공탁금 추가 버튼 이벤트
  ...
  loadAllAssets() {
	const types = [
	  ...
	  "exempt_property",
	  "exempt_property_special",
      "seizure_deposit", // (가)압류 적립금 타입 추가
      "deposit_money"    // 공탁금 타입 추가
	];
  ...
  populateAssetBlocks(type, assets) {
    ...
    switch (type) {
      ...
      case "exempt_property_special":
        ...
        break;
      case "seizure_deposit": // (가)압류 적립금 컨테이너 ID
        containerId = "#seizure_deposit_assets_container";
        break;
      case "deposit_money":   // 공탁금 컨테이너 ID
        containerId = "#deposit_money_assets_container";
        break;
      default:
  ...
  addAssetBlock(type, data = {}) {
    switch (type) {
      ...
      case "exempt_property_special":
        ...
        break;
      case "seizure_deposit": // (가)압류 적립금 블록 추가 호출
        this.addSeizureDepositBlock(data);
        break;
      case "deposit_money":   // 공탁금 블록 추가 호출
        this.addDepositMoneyBlock(data);
        break;
      default:
        console.warn("알 수 없는 자산 유형:", type);
        break;
...
saveVehicleBlock(block) {
  ...
  addRentDepositBlock(data = {}) {
    ...
    block.find(".rent_deposit_spouse").on("change", () => {
      ...
      if (isSpouseOwned) {
        // (반환받을 보증금 - 우선변제 보증금) / 2 계산
        const netValue = Math.max(0, refundDeposit - priorityDeposit);
        ...
        // 배우자명의가 아닌 경우 반환받을 보증금 - 우선변제 보증금
        const netValue = Math.max(0, refundDeposit - priorityDeposit);
        block.find(".rent_liquidation_value").val(this.formatMoney(netValue));
        block.find(".rent_liquidation_explain").val("");
      }
    });
    
    // 반환받을 보증금 및 우선변제 보증금 변경 이벤트 리스너 추가
    block.find(".rent_refund_deposit, .rent_priority_deposit").on("change", () => {
      const isSpouseOwned = block.find(".rent_deposit_spouse").is(":checked");
      const refundDeposit = this.unformatMoney(block.find(".rent_refund_deposit").val()) || 0;
      const priorityDeposit = this.unformatMoney(block.find(".rent_priority_deposit").val()) || 0;
  ...
  saveRentDepositBlock(block) {
    ...
    // 반환받을 보증금과 우선변제 보증금 가져오기
    const refundDeposit = this.unformatMoney(block.find(".rent_refund_deposit").val()) || 0;
    const priorityDeposit = this.unformatMoney(block.find(".rent_priority_deposit").val()) || 0;
    
    // 청산가치 계산
    let liquidationValue = this.unformatMoney(block.find(".rent_liquidation_value").val()) || 0;
    let explanation = block.find(".rent_liquidation_explain").val().trim();
    ...
    if (isSpouseOwned) {
      ...
      // 부연설명 업데이트
      explanation = "배우자명의 재산으로서 채무액을 공제한 환가예상액의 1/2 반영함";
      
      // UI 업데이트
      block.find(".rent_liquidation_value").val(this.formatMoney(liquidationValue));
      block.find(".rent_liquidation_explain").val(explanation);
    } else {
      // 배우자명의가 아닌 경우 반환받을 보증금 - 우선변제 보증금
      liquidationValue = Math.max(0, refundDeposit - priorityDeposit);
    }
...
saveRealEstateBlock(block) {
  ...
  addSeizureDepositBlock(data = {}) {
    ...
    const html = `
      <div class="asset-block seizure-deposit-block" id="${blockId}">
        ...
        <div class="content-wrapper">
          <div class="left-section">
            <div class="form">
              <div class="form-title"><span>적립기관</span></div>
              <div class="form-content">
                <input type="text" class="seizure_deposit_institution" value="${data.institution || ""}">
              </div>
            </div>
            <div class="form">
              <div class="form-title"><span>적립금액</span></div>
              <div class="form-content">
                <input type="text" class="seizure_deposit_amount" value="${data.amount ? this.formatMoney(data.amount) : ""}">원
              </div>
            </div>
          </div>
          <div class="right-section">
    ...
    const block = $("#" + blockId);
    block.find(".seizure_deposit_amount, .seizure_deposit_liquidation_value").on("input", (e) => {
      const val = e.target.value.replace(/[^\d]/g, "");
      e.target.value = this.formatMoney(val);
    });
  ...
  saveSeizureDepositBlock(block) {
    ...
    const data = {
      ...
      case_no: caseNo,
      institution: block.find(".seizure_deposit_institution").val().trim(),
      amount: this.unformatMoney(block.find(".seizure_deposit_amount").val()),
  ...
  // 14. 공탁금 섹션
  addDepositMoneyBlock(data = {}) {
    this.assetCounters.deposit_money++;
    const blockId = "deposit_money_block_" + this.assetCounters.deposit_money;
    const propertyNo = data.property_no || this.assetCounters.deposit_money;
    const html = `
      <div class="asset-block deposit-money-block" id="${blockId}">
        <input type="hidden" class="deposit_money_asset_no" value="${data.asset_no || ""}">
        <input type="hidden" class="deposit_money_property_no" value="${propertyNo}">
        <div class="content-wrapper">
          <div class="left-section">
            <div class="form">
              <div class="form-title"><span>공탁기관</span></div>
              <div class="form-content">
                <input type="text" class="deposit_money_institution" value="${data.institution || ""}">
              </div>
            </div>
            <div class="form">
              <div class="form-title"><span>공탁금액</span></div>
              <div class="form-content">
                <input type="text" class="deposit_money_amount" value="${data.amount ? this.formatMoney(data.amount) : ""}">원
              </div>
            </div>
          </div>
          <div class="right-section">
            <div class="form">
              <div class="form-title"><span>청산가치 판단금액</span></div>
              <div class="form-content">
                <input type="text" class="deposit_money_liquidation_value" value="${data.liquidation_value ? this.formatMoney(data.liquidation_value) : ""}">원
              </div>
            </div>
            <div class="form">
              <div class="form-title"></div>
              <div class="form-content btn-right">
                <button type="button" class="btn-delete deposit_money_delete_btn">삭제</button>
                <button type="button" class="btn-save deposit_money_save_btn">저장</button>
              </div>
            </div>
          </div>
        </div>
    ...
    `;
    $("#deposit_money_assets_container").append(html);
    const block = $("#" + blockId);
    block.find(".deposit_money_amount, .deposit_money_liquidation_value").on("input", (e) => {
      const val = e.target.value.replace(/[^\d]/g, "");
      e.target.value = this.formatMoney(val);
    });
    block.find(".deposit_money_save_btn").on("click", () => this.saveDepositMoneyBlock(block));
    block.find(".deposit_money_delete_btn").on("click", () => this.deleteDepositMoneyBlock(block));
  }

  saveDepositMoneyBlock(block) {
    const caseNo = window.currentCaseNo;
    const assetNo = block.find(".deposit_money_asset_no").val();
    const data = {
      asset_type: "deposit_money",
      case_no: caseNo,
      institution: block.find(".deposit_money_institution").val().trim(),
      amount: this.unformatMoney(block.find(".deposit_money_amount").val()),
      liquidation_value: this.unformatMoney(block.find(".deposit_money_liquidation_value").val()),
      property_no: block.find(".deposit_money_property_no").val()
    };
    if (assetNo) data.asset_no = assetNo;
    $.ajax({
      url: "api/application_recovery/assets/asset_api.php",
      ...
      success: (response) => {
        if (response.success) {
          alert("공탁금이 저장되었습니다.");
          block.find(".deposit_money_asset_no").val(response.data.asset_no);
        } else {
  ...
  deleteDepositMoneyBlock(block) {
    if (!block.find(".deposit_money_asset_no").val()) {
      block.remove();
      this.checkEmptyBlock("deposit_money");
      return;
    }
    if (!confirm("공탁금을 삭제하시겠습니까?")) return;
    const caseNo = window.currentCaseNo;
    const propertyNo = block.find(".deposit_money_property_no").val();
    $.ajax({
      url: "api/application_recovery/assets/asset_api.php",
      type: "DELETE",
      data: { asset_type: "deposit_money", case_no: caseNo, property_no: propertyNo },
      processData: true,
      contentType: "application/x-www-form-urlencoded; charset=UTF-8",
      dataType: "json",
      success: (response) => {
  ...
  checkEmptyBlock(assetType) {
    ...
    switch (assetType) {
      ...
      case "exempt_property_special":
        containerId = "#exempt_property_special_assets_container";
        break;
      case "seizure_deposit": // (가)압류 적립금 컨테이너 ID 추가
        containerId = "#seizure_deposit_assets_container";
        break;
      case "deposit_money":   // 공탁금 컨테이너 ID 추가
        containerId = "#deposit_money_assets_container";
        break;
      default:
