...
class FamilyMemberApi {
	...
	public function save($data) {
		try {
			...
			// 기존 레코드 확인 (member_no로)
			if (isset($data['member_no']) && !empty($data['member_no'])) {
				$stmt = $this->pdo->prepare("SELECT * FROM application_recovery_family_members WHERE member_no = ? AND case_no = ?");
				$stmt->execute([$data['member_no'], $data['case_no']]);
