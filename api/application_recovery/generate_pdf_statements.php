...
function generatePdfStatements($pdf, $pdo, $case_no) {
	...
	try {
		...
		$family_check = $is_family ? '■' : '□';
		$nonfamily_check = $is_nonfamily ? '■' : '□';
		...
		// 사유 체크박스 (기본으로 모두 체크된 상태로 표시)
		$pdf->Cell(0, 6, '( ■ )생활비 부족               ( ■ )병원비 과다지출', 0, 1, 'L');
		$pdf->Cell(0, 6, '( ■ )교육비 과다지출           ( ■ )음식,음주,여행,도박 또는 취미활동', 0, 1, 'L');
		$pdf->Cell(0, 6, '( ■ )점포 운영의 실패          ( ■ )타인 채무의 보증', 0, 1, 'L');
		$pdf->Cell(0, 6, '( ■ )주식투자 실패             ( ■ )사기 피해', 0, 1, 'L');
		$pdf->Cell(0, 6, '( ■ )기타 ( '.$detail.' )', 0, 1, 'L');
		...
		$pdf->Cell(50, 8, '■ 화의·회생·개인회생절차', 1, 0, 'L');
