...
function generatePdfIncome($pdf, $pdo, $case_no) {
	...
	try {
		...
		$col_width4 = 80; // 연간 환산 금액 및 압류 유무
		...
		$checkbox1 = ($living_expense_type == 'Y' || $living_expense_type == '') ? '■' : '□';
		$checkbox2 = ($living_expense_type == 'N') ? '■' : '□';
		...
		if (!empty($income_rows) || !empty($deduction_rows)) {
			...
			// 소득 테이블 헤더
			$pdf->Cell(20, 10, '지급내역', 1, 0, 'C');
			
			for ($i=1; $i<=12; $i++) {
				$pdf->Cell(13, 10, $i.'월', 1, 0, 'C');
			}
			
			$pdf->Cell(20, 10, '합계', 1, 1, 'C');
			...
			foreach ($income_rows as $row) {
				...
				for ($i=1; $i<=12; $i++) {
					...
					$pdf->Cell(13, 10, $month_value > 0 ? number_format($month_value) : '', 1, 0, 'R');
				}
				
				$pdf->Cell(20, 10, number_format($row_total), 1, 1, 'R');
				$total_income += $row_total;
			}
			
			// 소득 합계
			$pdf->Cell(20, 10, '소득합계', 1, 0, 'L');
			...
			// 월별 소득 합계 출력
			for ($i=1; $i<=12; $i++) {
				$pdf->Cell(13, 10, $monthly_income_totals[$i] > 0 ? number_format($monthly_income_totals[$i]) : '', 1, 0, 'R');
			}
			
			$pdf->Cell(20, 10, number_format($total_income), 1, 1, 'R');
			...
			// 월별 공제 합계 출력
			for ($i=1; $i<=12; $i++) {
				$pdf->Cell(13, 10, $monthly_deduction_totals[$i] > 0 ? number_format($monthly_deduction_totals[$i]) : '', 1, 0, 'R');
			}
			
			$pdf->Cell(20, 10, number_format($total_deduction), 1, 1, 'R');
			
			// 실수령액 행
			$pdf->Cell(20, 10, '실수령액', 1, 0, 'L');
			...
			// 월별 실수령액 출력
			for ($i=1; $i<=12; $i++) {
				$pdf->Cell(13, 10, $monthly_net_totals[$i] > 0 ? number_format($monthly_net_totals[$i]) : '', 1, 0, 'R');
			}
			
			$pdf->Cell(20, 10, number_format($net_total), 1, 1, 'R');
			...
			$pdf->Cell(45, 10, '연 공제총액(B)', 1, 0, 'L');
			$pdf->Cell(30, 10, number_format($yearly_deduction), 1, 0, 'R');
			$pdf->Cell(45, 10, '연 실수령액(A-B)', 1, 0, 'L');
			$pdf->Cell(30, 10, number_format($yearly_net), 1, 0, 'R');
			$pdf->Cell(45, 10, '월평균소득금액', 1, 0, 'L');
