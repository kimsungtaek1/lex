...
function generateExemptPropertyForm($pdf, $pdo, $case_no, $basic_info) {
	...
	// 첫 번째 MultiCell
	$pdf->MultiCell(25, 80, "주택임대차계약\n내용", 1, 'C', 0, 1, $x, $y, true, 0, false, true, 0, 'M', 1);


	// X, Y 위치 수동으로 오른쪽으로 이동해서 두 번째 셀 출력
	$pdf->SetXY($x + 25, $y);
	$pdf->MultiCell(0, 80, "①임대차계약일자   (                      )\n\n②임대차기간       (                      )\n\n③임차목적물의 소재지(                      )\n\n④임차보증금       (                      )\n\n⑤임료의 액수 및 연체기간(                      )\n\n⑥임대인의 성명    (                      )\n\n⑦주민등록일자     (                      )\n\n⑧확정일자(                      확정일자받음, 확정일자 무)", 1, 'L', false, 0);
	...
	// 면제재산 생계비 테이블
	$pdf->Cell(10, 10, '순번', 1, 0, 'C');
	$pdf->Cell(50, 10, '특정재산의 내용 (구체적으로 기재)', 1, 0, 'C');
	$pdf->Cell(30, 10, '소재지', 1, 0, 'C');
	$pdf->Cell(20, 10, '추정시가', 1, 0, 'C');
	...
	// 소명자료
	$pdf->Cell(20, 10, '※ 소명자료 :', 0, 0, 'L');
	$pdf->Cell(0, 10, '[   ] (         )보증서 1통/ [   ] 사진 1장/ [   ] 기타 [                ] 통', 0, 1, 'L');
