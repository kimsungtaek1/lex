...
$(document).ready(function() {
    ...
    function initApplicantEvents() {
        ...
        // 소득유형이 선택되어 있지 않으면 기본으로 급여소득자 선택 (id: salaryType)
        if (!$('input[name="incomeType"]:checked').length) {
            $("#salaryType").prop('checked', true).trigger('change');
        }
    }
    ...
    function fillApplicantForm(data) {
        ...
        // 소득 유형 설정  
        // DB에 is_company 값이 있으면 그에 따라, 없으면 기본으로 급여소득자(id: salaryType)를 선택
        if (data && typeof data.is_company !== 'undefined') {
            if (data.is_company === 1) {
                $("#businessType").prop('checked', true).trigger('change');
				// 섹션 토글: 자영 수입목록 보이고, (급여)수입목록 숨김
