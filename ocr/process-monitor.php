...
class OCRProcessMonitor {
     ...
     */
    public function createJob($name, $files, $userId, $options = []) {
        try {
            ...
            $stmt = $this->db->prepare("
                ...
                (name, user_id, total_files, processed_files, progress, status, options, created_at) 
                VALUES (?, ?, ?, 0, 0, 'queued', ?, NOW())
            ");
            
            $totalFiles = count($files);
            $optionsJson = json_encode($options);
            $stmt->execute([$name, $userId, $totalFiles, $optionsJson]);
            ...
            // 작업 로그 추가
            $this->addJobLog($jobId, 'info', "작업 생성: {$name}, 파일 수: {$totalFiles}");
     ...
     * @return array 작업 목록
     */
    public function getUserJobs($userId, $limit = 20, $offset = 0) {
        $stmt = $this->db->prepare("
            SELECT id, name, total_files, processed_files, progress, status, created_at, updated_at
            FROM ocr_jobs
            WHERE user_id = ?
