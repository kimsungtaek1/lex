...
class DocumentLearningSystem {
     ...
     * @return int 템플릿 ID
     */
    public function saveTemplate($templateData, $userId) {
        // 필수 필드 검증
        ...
        try {
            ...
            // 공개 여부
            $isPublic = !empty($templateData['is_public']) ? 1 : 0;
            ...
            if ($templateId) {
                ...
                $stmt = $this->db->prepare("
                    ...
                    (name, description, fields, table_structure, user_id, is_public, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
                ");
     ...
     * @return int 피드백 ID
     */
    public function saveFeedback($jobId, $fileId, $corrections, $userId) {
        try {
            // 피드백 저장
            $stmt = $this->db->prepare("
                INSERT INTO ocr_feedback
                (job_id, file_id, corrections, user_id, created_at)
                VALUES (?, ?, ?, ?, NOW())
            ");
            
            $correctionsJson = json_encode($corrections, JSON_UNESCAPED_UNICODE);
            $stmt->execute([$jobId, $fileId, $correctionsJson, $userId]);
        ...
        foreach ($corrections as $correction) {
            if (isset($correction['original']) && isset($correction['corrected'])) {
                ...
                if (!empty($originalText) && !empty($correctedText) && 
                    ...
                    // 기존 단어 확인
                    $stmt = $this->db->prepare("
                        SELECT id, similar_words FROM custom_dictionary
                        WHERE word = ?
                    ");
    ...
    private function getFrequentlyCorrectedFields() {
        // 모든 템플릿의 통계 분석
        $stmt = $this->db->prepare("
            SELECT learning_statistics FROM document_templates
